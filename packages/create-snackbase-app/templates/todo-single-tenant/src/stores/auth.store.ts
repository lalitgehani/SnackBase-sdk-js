...
interface AuthState {
  ...
  register: (email: string, password: string) => Promise<{ message: string; accountSlug: string }>;
  oauthLogin: (provider?: 'google' | 'github' | 'microsoft' | 'apple') => Promise<void>;
  logout: () => void;
  clearError: () => void;
  ...
  persist(
    (set, get) => ({
      ...
      oauthLogin: async (provider = 'google') => {
        ...
        try {
          const response: OAuthCallbackResponse = await oauthService.signInWithOAuth(provider);
