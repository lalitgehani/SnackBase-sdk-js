...
A simplified to-do application demonstrating SnackBase as a Backend-as-a-Service (BaaS) for single-tenant or internal use cases. Features email authentication and full CRUD operations for todos using the `@snackbase/sdk`.

## Features

- **Authentication**: Email/password registration and login with secure JWT tokens
- **Single-Tenant Focused**: Optimized for single-tenant deployments or simple internal tools
...
src/
├── lib/
│   ├── snackbase.ts    # SnackBase SDK client initialization
│   └── utils.ts        # Utility functions
├── types/
│   └── index.ts        # TypeScript interfaces
├── services/
│   ├── auth.service.ts # Auth service using SDK
│   ├── oauth.service.ts# OAuth service using SDK
│   └── todos.service.ts# Todo CRUD operations using SDK
├── stores/
...
3. User logs in → receives tokens via SDK
4. SDK automatically handles token injection and refresh
