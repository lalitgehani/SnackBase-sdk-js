...
export const workflowsTool: Tool = {
  ...
  description: 'Manage SnackBase workflows — multi-step automation pipelines. Create, trigger, and monitor workflow instances.',
  inputSchema: {
    type: 'object',
    properties: {
      action: {
        type: 'string',
        enum: ['list', 'get', 'create', 'update', 'delete', 'trigger', 'list_instances', 'get_instance', 'cancel_instance', 'retry_instance'],
        description: 'The action to perform on workflows.',
      },
      workflow_id: {
        type: 'string',
        description: 'The ID of the workflow (required for get, update, delete, trigger, list_instances, get_instance, cancel_instance, retry_instance).',
      },
      instance_id: {
        type: 'string',
        description: 'The ID of the workflow instance (required for get_instance, cancel_instance, retry_instance).',
      },
      name: {
        type: 'string',
...
export async function handleWorkflowsTool(args: any) {
  ...
  const { action, workflow_id, instance_id, name, trigger, steps, enabled, input, limit, skip } = args;

  try {
    switch (action) {
      case 'list':
        const workflows = await client.workflows.list({ limit, skip });
        return {
      ...
      case 'update':
        ...
        const updatedWorkflow = await client.workflows.update(workflow_id, { name, trigger, steps, enabled });
        return {
      ...
      case 'list_instances':
        ...
        const instances = await client.workflows.listInstances(workflow_id, { limit, skip });
        return {
          content: [{ type: 'text', text: JSON.stringify(instances, null, 2) }],
        };

      case 'get_instance':
        if (!workflow_id || !instance_id) throw new Error('workflow_id and instance_id are required for get_instance action');
        const workflowInstance = await client.workflows.getInstance(workflow_id, instance_id);
        return {
          content: [{ type: 'text', text: JSON.stringify(workflowInstance, null, 2) }],
        };

      case 'cancel_instance':
        if (!workflow_id || !instance_id) throw new Error('workflow_id and instance_id are required for cancel_instance action');
        const cancelledInstance = await client.workflows.cancelInstance(workflow_id, instance_id);
        return {
          content: [{ type: 'text', text: JSON.stringify(cancelledInstance, null, 2) }],
        };

      case 'retry_instance':
        if (!workflow_id || !instance_id) throw new Error('workflow_id and instance_id are required for retry_instance action');
        const retriedInstance = await client.workflows.retryInstance(workflow_id, instance_id);
        return {
